...
class paint:	
	...
	def setmouse(self, x, y):
		print('To move 1 pixel,  a:left\n d:right\n w:up\n s:down')
		print('To move 10 pixels,  j:left\n l:right\n i:up\n k:down')
		while True:
			try:
				c=getch().decode()
			except:
		...
		while True:
			...
			while True:
				...
				if ulx>=width:
					break
				if flag==1:
					xr = self.drift('right',sz=scale)
				else:
					xr = self.drift('left',sz=scale)
			uly = uly + (400-yr)//scale
			if uly>=height:
				break
			yr = self.drift('down',sz=scale)
	...
	def autowrite(self, text, fontsize=12):
		...
		while True:
			...
			while True:
					...
					for i in range(colnum):
						...
						if c!=' ':
							if fontsize>30:
								...
								sp=7
							self.Hershey(textblock[j][i],llx,lly,2*fontsize,3*fontsize,speed=sp)
							time.sleep(0.05)
				colcount+=colnum
				...
				if flag==1:
					xr = self.drift('right',sz=psz)
				else:
					xr = self.drift('left',sz=psz)
			rowcount += rownum
	...
	def Hershey(self,s,llx,lly,scx,scy,speed=7):
		fn=open('chars.txt').read().split('\n')
		num=len(s)
		i=0
		width=int(scx/num)
		for char in s:
			...
			ind=ord(char)-32
			letter=fn[ind].split(',')
			self.typeletter(letter,x,y,width,scy,speed)
			i+=1
	...
	def MeteorShower(self):
		...
		if c=='1':
			...
			if n==0:
				n=3
				w='LOL'
		print('How many prints per row(default is 20)?')
		c=input()
		try:
			num=int(c)
		except:
		...
		try:
			...
			sp=7
		print('Input the target color, (use rgb values, e.g. 255,0,0), default is red and pink.')
		while True:
			try:
				r,g,b=input().split(',')
				r=int(r)
		...
		while True:
			h_c+=1
			a+=self.autoplot(target,mode=m,rtl=0.9/n, rtu=1.5/n, s=w,speed=sp)
			time.sleep(1)
			if h_c == num:
				h_c=0
				v_c+=1
